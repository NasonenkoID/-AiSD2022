...
class ProjectileMotion
{
    ...
    public delegate void TrajectoryUpdateEventHandler(double positionX, double positionY);
    public static event TrajectoryUpdateEventHandler TrajectoryUpdate;
        ...
        using (StreamWriter writer = new StreamWriter(outputFileName))
        {
            while (positionX <= obstaclePositionX && positionY >= 0)
            {
                ...
                if (positionX <= obstaclePositionX && positionY >= 0)
                {
                    ...
                    TrajectoryUpdate?. Invoke(positionX, positionY);
                }
                else if (positionX > obstaclePositionX)
                {
                    writer.WriteLine($"Препятствие достигнуто на координате X: {positionX:F2} м.");
                    Collision?.Invoke(null, new CollisionEventArgs(true));
                    break;
                }
                else if (positionY < 0)
                {
                    writer.WriteLine("Тело упало на землю.");
                    Collision?.Invoke(null, new CollisionEventArgs(false));
                    break;
                }
